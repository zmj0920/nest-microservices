...
import { PublishStatus } from '../task/task.mysql.entity';
...
export class MaterialController {
  ...
  async publish(@Body() publishDto: PublishDto, @PayloadUser() user: Payload) {
    ...
    const projectType =
      project.projectTypes.length > 1 ? 'cnpm' : project.projectTypes[0];
    ...
    if (environment !== ProcessNodes.production) {
      if (
        ...
        deployVersion = `${deployVersion}-${versionTypeMap[environment]}.${
          material[versionMap[environment]] + 1
        }`;
